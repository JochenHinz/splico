...
def aspect_ratio(mesh: Mesh) -> Tuple[np.float_, ...]:
  ...
  This function calculates the aspect ratio (AR) for each element
  in the mesh, defined as the ratio between the longest and shortest
  edge in that element. The aspect ratio is a measure of mesh quality:
  - AR close to 1 indicates a well-proportioned, high-quality mesh 
     element
  ...
  Notes:
   ...
   - For 2D elements (e.g., triangles, quads), the aspect ratio is based 
     on edge lengths.
